...
function makeService(db) {
  ...
  async function createSeason(opts) {
    ...
    var sn = await db.createSeason({ code: code, name: opts.name || null, matchTarget: target });
    return { code: sn.code, name: sn.name, matchTarget: sn.match_target, status: sn.status };
  }

  /* Running totals across every finished match in a season. Reads the
     `scores` table, which existed unused since Phase 1 and is now the point. */
  async function seasonStandings(seasonCode) {
    var sn = await db.getSeasonByCode(String(seasonCode).toUpperCase());
  ...
  return {
    ...
    replay: replay, matchRecap: matchRecap, createSeason: createSeason, seasonStandings: seasonStandings,
