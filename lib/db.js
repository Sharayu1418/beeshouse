...
function memoryDb() {
  ...
  return {
    ...
    /* The most recently created room that has not finished. There is exactly
       one group, so "is a game running" is a question about the whole app
       rather than about a device. */
    async findOpenGame() {
      var open = [];
      games.forEach(function (g) { if (g.status !== 'done') open.push(g); });
      open.sort(function (a, b) { return String(a.created_at) < String(b.created_at) ? -1 : 1; });
      return open.length ? clone(open[open.length - 1]) : null;
    },
    async lastMoveAt(gameId) {
      var mine = moves.filter(function (m) { return m.game_id === gameId; });
      return mine.length ? mine[mine.length - 1].created_at : null;
    },
...
function pgDb(connectionString) {
  ...
  return {
    ...
    async findOpenGame() {
      var r = await q("select * from games where status <> 'done' order by created_at desc limit 1");
      return r.rows[0] || null;
    },
    async lastMoveAt(gameId) {
      var r = await q('select created_at from moves where game_id = $1 order by id desc limit 1', [gameId]);
