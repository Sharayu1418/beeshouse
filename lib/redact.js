...
/* Card the whole table can see. */
function publicCard(c) {
  return c ? { id: c.id, v: c.v } : null;
}
...
function viewFor(state, seat, meta) {
  ...
  var view = {
    ...
    drawn: (isMyTurn && state.drawn)
      ? { id: state.drawn.card.id, v: state.drawn.card.v, from: state.drawn.from }
      : null,
...
function revealFor(state, seat) {
  ...
  if (m.kind === 'reveal') {
    return { kind: 'card', title: m.title, sub: m.sub, v: m.card.v, id: m.card.id };
  }
  if (m.kind === 'listenResult') {
    return { kind: 'answer', q: m.q, ans: m.ans, who: m.who };
  }
  if (m.kind === 'neck') {
    // Long Neck earns sight of the top three, once
    return {
      kind: 'cards',
      title: 'Long Neck',
      cards: state.deck.slice(0, 3).map(publicCard)
    };
  }
  return null;
}
...
function assertNoLeak(payload, state, seat) {
  ...
  (function walk(node, path) {
    ...
    // a card-shaped object carrying a value
    if (typeof node.v === 'number' && typeof node.id === 'string' && /^c\d+$|^p\d+$/.test(node.id)) {
      if (!allowed.has(node.id)) problems.push(path + ' exposes card ' + node.id + ' (value ' + node.v + ')');
    }
